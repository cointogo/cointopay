                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 z2PAY ought to be enough for anybody." - Bill Gates, 2017
    ui->z2PAYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelz2PAYSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->z2PAYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZIJA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z2PAY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_z2PAY, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " z2PAY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" z2PAY in ") +
...
void PrivacyDialog::on_pushButtonSpendzZIJA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z2PAY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_z2PAY, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn z2PAY
        sendz2PAY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send z2PAY
    sendz2PAY();
}

void PrivacyDialog::on_pushButtonZCointopayControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZCointopayControlDialog* zCointopayControl = new ZCointopayControlDialog(this);
    zCointopayControl->setModel(walletModel);
    zCointopayControl->exec();
}

void PrivacyDialog::setZCointopayControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCointopaySelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendz2PAY()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Cointopay Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->z2PAYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as z2PAY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " 2PAY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " z2PAY</b>";
    ...
    // use mints from z2PAY selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCointopayControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCointopayControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 z2PAY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend z2PAY"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If z2PAY was spent successfully update the addressbook with the label
    ...
    ZCointopayControlDialog::setSelectedMints.clear();
    ui->labelzCointopaySelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("z2PAY Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Cointopay, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("z2PAY Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " z2PAY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" z2PAY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" z2PAY "));
    ui->labelz2PAYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>z2PAY </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " z2PAY </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelz2PAYSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintz2PAY->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint z2PAY
        ui->pushButtonMintz2PAY->setEnabled(false);
        ui->pushButtonMintz2PAY->setToolTip(tr("z2PAY is currently disabled due to maintenance."));

        // Spend z2PAY
        ui->pushButtonSpendz2PAY->setEnabled(false);
        ui->pushButtonSpendz2PAY->setToolTip(tr("z2PAY is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint z2PAY
        ui->pushButtonMintz2PAY->setEnabled(true);
        ui->pushButtonMintz2PAY->setToolTip(tr("PrivacyDialog", "Enter an amount of 2PAY to convert to z2PAY", 0));

        // Spend z2PAY
        ui->pushButtonSpendz2PAY->setEnabled(true);
        ui->pushButtonSpendz2PAY->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

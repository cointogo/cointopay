...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::THE2PAY);
    ...
    if (!settings.contains("nAnonymizeCointopayAmount"))
        settings.setValue("nAnonymizeCointopayAmount", 1000);

    nAnonymizeCointopayAmount = settings.value("nAnonymizeCointopayAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCointopayAmount"))
        SoftSetArg("-anonymizecointopayamount", settings.value("nAnonymizeCointopayAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cointopay.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCointopayAmount:
            return QVariant(nAnonymizeCointopayAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCointopayAmount:
            nAnonymizeCointopayAmount = value.toInt();
            settings.setValue("nAnonymizeCointopayAmount", nAnonymizeCointopayAmount);
            emit anonymizeCointopayAmountChanged(nAnonymizeCointopayAmount);
            break;
        case CoinControlFeatures:

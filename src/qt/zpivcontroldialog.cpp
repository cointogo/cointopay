...
std::set<std::string> ZCointopayControlDialog::setSelectedMints;
std::set<CMintMeta> ZCointopayControlDialog::setMints;

ZCointopayControlDialog::ZCointopayControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCointopayControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCointopayControlDialog::~ZCointopayControlDialog()
{
    delete ui;
}

void ZCointopayControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCointopayControlDialog::updateList()
{
...
void ZCointopayControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCointopayControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCointopay_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCointopayControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCointopayControlDialog::GetSelectedMints()
{
...
void ZCointopayControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

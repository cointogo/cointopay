...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpivTracker = unique_ptr<Cz2PAYTracker>(new Cz2PAYTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZZIJA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZZIJA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");

...
Cz2PAYTracker::Cz2PAYTracker(std::string strWalletFile)
{
...
Cz2PAYTracker::~Cz2PAYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void Cz2PAYTracker::Init()
{
...
bool Cz2PAYTracker::Archive(CMintMeta& meta)
{
...
bool Cz2PAYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta Cz2PAYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta Cz2PAYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool Cz2PAYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> Cz2PAYTracker::GetSerialHashes()
{
...
CAmount Cz2PAYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount Cz2PAYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> Cz2PAYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool Cz2PAYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool Cz2PAYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool Cz2PAYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool Cz2PAYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool Cz2PAYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool Cz2PAYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool Cz2PAYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void Cz2PAYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz2PAYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz2PAYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void Cz2PAYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void Cz2PAYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool Cz2PAYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> Cz2PAYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {

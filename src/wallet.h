...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 2PAY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for z2PAY send
enum ZerocoinSpendStatus {
    ...
    ZZIJA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZIJA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZIJA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZIJA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZIJA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZIJA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZIJA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZIJA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZIJA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ...
    ZZIJA_SPENT_USED_Z2PAY = 14,                      // Coin has already been spend
...
public:
    ...
    void ZCointopayBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZ2PAYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    Cz2PAYWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<Cz2PAYTracker> zpivTracker;
    ...
    void setZWallet(Cz2PAYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<Cz2PAYTracker>(new Cz2PAYTracker(strWalletFile));
    }

    Cz2PAYWallet* getZWallet() { return zwalletMain; }
    ...
    void setZCointopayAutoBackups(bool fEnabled)
    {
    ...
    /** z2PAY reset */
    boost::signals2::signal<void()> Notifyz2PAYReset;

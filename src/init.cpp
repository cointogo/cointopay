...
#include "config/cointopay-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("cointopay-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cointopay.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cointopayd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the 2PAY and z2PAY money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 59101, 17111));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in 2PAY/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in 2PAY/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, cointopay, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in 2PAY/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all 2PAY specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-cointopaystake=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-cointopaystake=<n>", strprintf(_("Enable or disable staking functionality for 2PAY inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcointopaystake=<n>", strprintf(_("Enable or disable staking functionality for z2PAY inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "173.249.48.233:59101"));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each z2PAY minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic z2PAY backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizecointopayamount=<n>     " + strprintf(_("Keep N 2PAY anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 59102, 51995));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The HUZU Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The 2PAY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("cointopay-loadblk");
...
/** Sanity checks
 *  Ensure that 2PAY is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
#endif
        if (SoftSetBoolArg("-cointopaystake", false))
            LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -cointopaystake=0\n");
#ifdef ENABLE_WALLET
    }
#endif
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. 2PAY Core is shutting down."));
...
#endif
    // Make sure only a single 2PAY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. 2PAY Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //2PAY: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //2PAY specific: zerocoin and spork DB's
                ...
                // 2PAY: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZ2PAYMinted();
                        RecalculateZ2PAYSpent();
                    }
                    Recalculate2PAYSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // 2PAY: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of 2PAY Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart 2PAY Core to complete") << "\n";
        ...
        zwalletMain = new Cz2PAYWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize z2PAYWallet
        uiInterface.InitMessage(_("Syncing z2PAY wallet..."));

        bool fEnableZCointopayBackups = GetBoolArg("-backupzpiv", true);
        pwalletMain->setZCointopayAutoBackups(fEnableZCointopayBackups);
...
//    }
//
//    nAnonymizeCointopayAmount = GetArg("-anonymizecointopayamount", 0);
//    if (nAnonymizeCointopayAmount > 999999) nAnonymizeCointopayAmount = 999999;
//    if (nAnonymizeCointopayAmount < 2) nAnonymizeCointopayAmount = 2;
    ...
    LogPrintf("Anonymize 2PAY Amount %d\n", nAnonymizeCointopayAmount);
    ...
    /* Denominations
       ...
       For example:
       12PAY+1000 == (.12PAY+100)*10
       102PAY+10000 == (12PAY+1000)*10
    */

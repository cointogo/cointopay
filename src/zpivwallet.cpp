...
Cz2PAYWallet::Cz2PAYWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZ2PAYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZ2PAYSeed_deprecated()) {
                    LogPrintf("%s: Updated z2PAY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool Cz2PAYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZ2PAYCount(nCountLastUsed);
    else if (!walletdb.ReadZ2PAYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void Cz2PAYWallet::Lock()
{
    seedMaster = 0;
}

void Cz2PAYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void Cz2PAYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZ2PAY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool Cz2PAYWallet::LoadMintPoolFromDB()
{
...
void Cz2PAYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void Cz2PAYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void Cz2PAYWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool Cz2PAYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZ2PAY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void Cz2PAYWallet::SeedToZ2PAY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 Cz2PAYWallet::GetZerocoinSeed(uint32_t n)
{
...
void Cz2PAYWallet::GenerateDeterministicZ2PAY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void Cz2PAYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool Cz2PAYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
